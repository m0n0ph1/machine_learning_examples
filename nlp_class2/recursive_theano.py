...
class RecursiveNN:
    ...
    def fit(self, trees, learning_rate=3 * 1e-3, mu=0.99, reg=1e-4, epochs=15, activation=T.nnet.relu,
        ...
        for i in range(epochs):
            ...
            for j in sequence_indexes:
                ...
                it += 1
                if it % 1 == 0:
                    sys.stdout.write("j/N: %d/%d correct rate so far: %f, cost so far: %f\r" % (
...
def tree2list(tree, parent_idx, is_binary=False, is_left=False, is_right=False):
    ...
    words_right, parents_right, relations_right, labels_right = tree2list(tree.right, tree.idx, is_binary,
                                                                          is_right=True)
