...
class RNN:
    ...
    def fit(self, X, learning_rate=1e-4, mu=0.99, epochs=10, batch_sz=100, show_fig=True, activation=T.nnet.relu,
            RecurrentUnit=LSTM):
        D = self.D
        V = self.V
        N = len(X)
        ...
        for i in range(epochs):
            ...
            for j in range(n_batches):
                ...
                if j % 1 == 0:
                    sys.stdout.write(
                        "j/n_batches: %d/%d correct rate so far: %f\r" % (j, n_batches, float(n_correct) / n_total))
                    sys.stdout.flush()
